...
import {
	...
	incrementSell,
	incrementView,
} from "./server/DB/functions.js";
import { error } from "console";
import { produtos } from "./server/DB/structure.js";
...
app.put("/api/cart/checkout", async (req, res) => {
	...
	if ((await Promise.all(estoque)).includes(false)) {
		...
		productsOnDB.forEach(async (product, index) => {
			...
			await produtos.update(
				{
					vendas: Sequelize.literal(
						`vendas + ${body[index].quantity}`
					),
				},
				{ where: { id: product.id } }
			);
...
app.use(
	"/api/productImages",
	express.static("./src/server/public/imgs/produtos")
);
app.use(
	"/api/productImagesTransparent",
	express.static("./src/server/public/imgs/produtosTransparent")
);
